...
// scripts/captureDocs.bs.js
var captureDocs_bs_exports = {};
__export(captureDocs_bs_exports, {
  Docgen: () => Docgen,
  capitalize: () => capitalize,
  configFile_decode: () => configFile_decode,
  configFile_encode: () => configFile_encode,
...
module.exports = __toCommonJS(captureDocs_bs_exports);
  ...
  if (len_a === len_b) {
    if (Array.isArray(a)) {
      ...
      var do_key_b = function(key2) {
        if (!Object.prototype.hasOwnProperty.call(a, key2) || !equal(b[key2], a[key2])) {
          result.contents = false;
          return;
        }
      };
...
function map(arg1, obj) {
  return obj.map(__1(arg1));
}
function reducei(arg1, arg2, obj) {
  return obj.reduce(__3(arg1), arg2);
}
...
// node_modules/@greenlabs/ppx-spice/src/rescript/Spice.bs.js
function error(path, message, value) {
  var path$1 = path !== void 0 ? path : "";
  return {
    TAG: "Error",
    _0: {
      path: path$1,
...
function arrayFromJson(decoder, json) {
  if (!Array.isArray(json) && (json === null || typeof json !== "object") && typeof json !== "number" && typeof json !== "string" && typeof json !== "boolean" || !Array.isArray(json)) {
    ...
    return reducei(function(acc, jsonI, i) {
...
function filterOptional(arr) {
  return map2(keep(arr, function(param) {
    return !(param[1] && equal(param[2], null));
  }), function(param) {
    return [
      param[0],
      param[2]
...
// node_modules/@rescript/core/src/Core__JSON.bs.js
function bool(json) {
...
// scripts/createHome.bs.js
var Fs = __toESM(require("fs"));

// node_modules/@rescript/core/src/Core__Dict.bs.js
...
// node_modules/@rescript/core/src/Core__Array.bs.js
function reduce2(arr, init, f) {
  return arr.reduce(f, init);
}
function findIndexOpt(array2, finder) {
  var index2 = array2.findIndex(finder);
  if (index2 !== -1) {
    return index2;
  }
}

// node_modules/@rescript/core/src/Core__Option.bs.js
function mapOr(opt, $$default2, f) {
  if (opt !== void 0) {
...
// scripts/createHome.bs.js
function dummy(destinationFilePath) {
  Fs.writeFileSync(destinationFilePath, "\n@react.component\nlet make = () => {\n  <div/>\n}\n  ");
}
function create(toc_tree, destinationPath, sources) {
  var sidebarInput = [];
  forEachWithKey(toc_tree, function(v, k) {
    var groupName = getOr(flatMap(getOr(Decode.object(v), {})["groupName"], Decode.string), "");
    var idx = findIndexOpt(sidebarInput, function(e) {
      return e.groupName === groupName;
    });
    if (idx !== void 0) {
      var g = sidebarInput[idx];
      var el = {
        groupName,
  ...
  var input = "[" + sidebarInput.map(function(e) {
    var inputs = "[" + e.inputs.map(function(i) {
      var componentName = i.label + "_Gen_Doc.make";
      return '{label: "' + i.label + '", onSelect: < ' + componentName + " />}";
    }).join(",") + "]";
    return '{groupName: "' + e.groupName + '", inputs: ' + inputs + "}";
  }).join(",") + "]";
  var content3 = "\n    @react.component\n    let make = () => {\n      <CF_Sidebar inputGroups={" + input + "}/>\n    }\n    ";
  Fs.writeFileSync(destinationPath, content3);
}
...
function getWithDefault(opt, $$default2) {
  if (opt !== void 0) {
    return valFromOption(opt);
  } else {
    return $$default2;
      ...
      case OBJECT: {
        ...
        for (const key2 of keys(value)) {
          if (strict || !shouldSkip(typeOf(value[key2])))
            entries2.push([pair(key2), pair(value[key2])]);
        }
        return index2;
      }
      case DATE:
      ...
      case MAP: {
        ...
        for (const [key2, entry] of value) {
          if (strict || !(shouldSkip(typeOf(key2)) || shouldSkip(typeOf(entry))))
            entries2.push([pair(key2), pair(entry)]);
        }
        return index2;
      }
      case SET: {
...
function create2(definition2) {
  ...
  for (prop in definition2.properties) {
    if (own2.call(definition2.properties, prop)) {
    ...
    if (state.verbose) {
      ...
      if (location2.attrs) {
        for (key2 in location2.attrs) {
          if (own4.call(location2.attrs, key2)) {
...
var Parser = class {
  ...
  static parse(html6, options) {
    ...
    parser.tokenizer.write(html6, true);
...
function aaObtainFurthestBlock(p2, formattingElementEntry) {
  ...
  for (; idx >= 0; idx--) {
    const element5 = p2.openElements.items[idx];
    if (element5 === formattingElementEntry.element) {
      break;
    }
    if (p2._isSpecialElement(element5, p2.openElements.tagIDs[idx])) {
...
function endTagInForeignContent(p2, token) {
  ...
  for (let i = p2.openElements.stackTop; i > 0; i--) {
    ...
    const tagName = p2.treeAdapter.getTagName(element5);
...
// node_modules/@rescript/tools/npm/Tools_Docgen.bs.js
...
function all4(values, includeImageAlt, includeHtml) {
  const result = [];
  let index2 = -1;
  while (++index2 < values.length) {
...
function decodeNamedCharacterReference(value) {
  return own7.call(characterEntities, value) ? characterEntities[value] : false;
}
...
function tokenizeHtmlText(effects, ok3, nok) {
  ...
  function commentClose(code2) {
    ...
    return comment4(code2);
  }
  function commentEnd(code2) {
    return code2 === 62 ? end(code2) : code2 === 45 ? commentClose(code2) : comment4(code2);
  }
  function cdataOpenInside(code2) {
    const value = "CDATA[";
    if (code2 === value.charCodeAt(index2++)) {
  ...
  function constructFactory(onreturn, fields) {
    ...
    function hook(constructs2, returnState, bogusState) {
      ...
      function handleMapOfConstructs(map7) {
        return start;
        function start(code2) {
          const def = code2 !== null && map7[code2];
          const all6 = code2 !== null && map7.null;
          const list3 = [
...
function createState(tree, options) {
  ...
  function one5(node2, parent) {
    ...
    if (own9.call(state.handlers, type) && handle2) {
...
function comment3(node2, _12, _2, state) {
...
function doctype3(_12, _2, _32, state) {
...
function omission(handlers2) {
  ...
  function omit(node2, index2, parent) {
    return own11.call(handlers2, node2.tagName) && handlers2[node2.tagName](node2, index2, parent);
  }
}
...
function root4(node2, _12, _2, state) {
...
var handle = zwitch("type", {
  invalid,
  unknown: unknown2,
  handlers: { comment: comment3, doctype: doctype3, element: element4, raw: raw2, root: root4, text: text6 }
});
...
// scripts/generateMdx.bs.js
function encodeCodeString(code2) {
  return code2.replaceAll(/\"/g, '\\"');
}
var astTransform = function mapper(ast, parent) {
  let regex = /%\(([\s\S]*?)\)%/g;
  let parseAndRenderCode = (line) => {
    let code2 = line.replace(regex, "$1");
    return '<CF_CodeWrapper code={"' + encodeCodeString(code2) + '"} lang={"res"} > {' + code2 + "} </CF_CodeWrapper>";
  };
  let mapChildren = () => {
...
async function stylize(content3) {
  var rendered = async (unified2, remarkParse2, remarkRehype2, rehypeRaw2, rehypeStringify2, content4) => {
    function logContent() {
      return (tree) => {
    ...
    const file = await unified2().use(remarkParse2).use(remarkRehype2, { "allowDangerousHtml": true }).use(logContent).use(rehypeStringify2, { "allowDangerousHtml": true }).process(content4);
    return String(file);
  };
  return await rendered(unified, remarkParse, remarkRehype, rehypeRaw, rehypeStringify, content3);
}
function getModuleJson(moduleName, mainModule, tree) {
  return entries(mainModule).map(function(param) {
    var content3 = param[1];
    var match = decodeFromJson(content3);
    var id;
    if (!Array.isArray(content3) && (content3 === null || typeof content3 !== "object") && typeof content3 !== "number" && typeof content3 !== "string" && typeof content3 !== "boolean" || !(typeof content3 === "object" && !Array.isArray(content3))) {
      id = "";
    } else {
      var match$1 = get(content3, "id");
      id = match$1 !== void 0 && !(!Array.isArray(match$1) && (match$1 === null || typeof match$1 !== "object") && typeof match$1 !== "number" && typeof match$1 !== "string" && typeof match$1 !== "boolean" || typeof match$1 !== "string") ? match$1 : "";
    }
    var items = match.items.map(function(item) {
      switch (item.kind) {
        case "value":
          return {
            TAG: "Value",
            ...
            name: item.name,
            deprecated: fromOption(item.deprecated)
          };
        case "type":
          ...
          if (detail !== void 0) {
            if (detail.kind === "record") {
              var items2 = detail.items.map(function(param2) {
                return {
                  name: param2.name,
                  docstrings: param2.docstrings,
                  signature: param2.signature,
                  optional: param2.optional,
                  deprecated: fromOption(param2.deprecated)
                };
              });
              detail$1 = {
                TAG: "Record",
              ...
              var items$1 = detail.items.map(function(param2) {
                return {
                  name: param2.name,
                  docstrings: param2.docstrings,
                  signature: param2.signature,
    ...
    var module__docstrings = match.docstrings;
    var module__deprecated = fromOption(match.deprecated);
    var module__name = match.name;
    var module_ = {
      id,
      docstrings: module__docstrings,
      deprecated: module__deprecated,
      name: module__name,
      items
    };
    var toctree = get(tree, moduleName);
    if (toctree !== void 0) {
      return {
        TAG: "Ok",
        _0: {
          module_,
          toctree: valFromOption(toctree)
        }
      };
    } else {
      return {
        TAG: "Error",
        _0: "Failed to find toctree to " + param[0]
      };
    }
  });
}
async function encodeDocstrings(s2) {
  var data = await stylize(s2.join("\\n").replace(/\\n/g, "\n").replace(/\\\"/g, '"'));
  if (data !== "") {
    return '<div className="my-4"> ' + data + " </div>";
  } else {
    return "";
  }
}
async function parseDocFileWithAST(moduleName, docFileAST, tree, docFileContent) {
  var modules = getModuleJson(moduleName, docFileAST, tree).slice(1);
  var getAllDocStrings = function(module_) {
    if (module_.TAG !== "Ok") {
      return [];
    }
    var match = module_._0.module_;
    return match.docstrings.concat(reduce2(match.items, [], function(acc, item) {
      return acc.concat(item.docstrings);
    }));
  };
  var content3 = await Promise.all(modules.map(async function(mod) {
    var allDocStrings = getAllDocStrings(mod);
    var encodedDocStrings = await encodeDocstrings(allDocStrings);
    var encodedComponent;
    if (mod.TAG === "Ok") {
      var id = mod._0.module_.id;
      encodedComponent = id.split(".").length === 2 ? '<CF_CodeWrapper code={"' + encodeCodeString(docFileContent) + '"} lang={"res"}> <' + id + "/> </CF_CodeWrapper>" : "";
    } else {
      encodedComponent = "";
    }
    return encodedDocStrings + "\n" + encodedComponent;
  }));
  return reduce2(content3, "", function(acc, el) {
    return acc + "\n" + el;
  });
}
async function $$default(moduleName, mainModule, tree, destinationFilePath, docFileAST, docFileContent) {
  var moduleResult = getModuleJson(moduleName, mainModule, tree);
  var encodeCodeString2 = function(code2) {
    return code2.replaceAll(/\"/g, '\\"');
  };
  var codeExampleString = docFileAST !== void 0 ? await parseDocFileWithAST(moduleName, valFromOption(docFileAST), tree, docFileContent) : "";
  var mdxContent = await Promise.all(moduleResult.map(async function(result) {
    if (result.TAG !== "Ok") {
      return "";
    }
    var match = result._0.module_;
    var encodedDocStrings = await encodeDocstrings(match.docstrings);
    var valuesAndType = await Promise.all(match.items.map(async function(item) {
      if (item.TAG === "Value") {
        var encodedDocStrings2 = await encodeDocstrings(item.docstrings);
        var code2 = item.signature.replace(/\\n/g, "\n");
        return '\n<Markdown.H2.make> {"' + item.name + '"->React.string} </Markdown.H2.make>\n< CF_CodeExample code={"' + encodeCodeString2(code2) + '"} lang="res" />\n' + encodedDocStrings2;
      }
      var encodedDocStrings$1 = await encodeDocstrings(item.docstrings);
      var code$1 = item.signature.replace(/\\n/g, "\n");
      return '\n<Markdown.H2.make> {"' + item.name + '"->React.string} </Markdown.H2.make>\n/*  <DeprecatedMessage deprecated="' + item.deprecated + '"/> */\n< CF_CodeExample code={"' + encodeCodeString2(code$1) + '"} lang="res" />\n' + encodedDocStrings$1;
    }));
    return '\n<Markdown.H1.make> {"' + match.name + '"->React.string} </Markdown.H1.make>\n' + encodedDocStrings + "\n" + valuesAndType.join("\n");
  }));
  var rescriptContent = '\n@react.component\nlet make = () => {\n   <div className="gap-6 flex flex-col">\n   <div> ' + codeExampleString + " </div>\n   " + mdxContent.join("") + "\n   </div>\n}\n ";
  Fs2.writeFileSync(Path.join(destinationFilePath), rescriptContent);
}

// node_modules/@rescript/core/src/Core__Result.bs.js
function map6(opt, f) {
  if (opt.TAG === "Ok") {
    return {
      TAG: "Ok",
...
var getAllFiles = function getAllFiles2(dirPath) {
  let isFile = (path) => {
    try {
      const stats = Fs3.statSync(path);
      return stats.isFile();
    } catch (err) {
      return false;
    }
  ...
  if (isFile(dirPath)) {
    ...
    files.forEach(function(file) {
      ...
      if (file == "node_modules" || file == "dist" || file == "lib" || file == ".yarn") {
      } else {
        if (Fs3.statSync(child).isDirectory()) {
          arrayOfFiles = arrayOfFiles.concat(getAllFiles2(dirPath + "/" + file));
        } else if (Path2.extname(file) === ".res") {
...
function run(destinationPath, entryPointFiles, platform, isDocFileOpt) {
  var isDocFile = isDocFileOpt !== void 0 ? isDocFileOpt : false;
  var docsDecoded = entryPointFiles.map(function(param) {
    var output = Child_process.execSync("./node_modules/@rescript/tools/binaries/" + platform + "/rescript-tools.exe  doc " + param[1]).toString();
    return [
      param[0],
      decodeFromJson(JSON.parse(output))
    ];
  });
  var docs = docsDecoded.map(function(param) {
    var doc = param[1];
    var group = param[0];
    var topLevelItems = keepMap(doc.items, function(item) {
      switch (item.kind) {
        case "value":
        case "type":
          return item;
    ...
    var getModules2 = function(_lst, _moduleNames) {
      while (true) {
        ...
        var rest = lst.tl;
        if (hiddenModules.includes(id2)) {
          _lst = rest;
          continue;
        }
        var id$1 = id2.startsWith("RescriptCore") ? id2.replace("RescriptCore", "Core") : id2;
        _moduleNames = {
          hd: {
            id: id$1,
            docstrings,
    ...
    var id = doc.name.startsWith("RescriptCore") ? doc.name.replace("RescriptCore", "Core") : doc.name;
    ...
    var submodules = toArray(getModules2(
      fromArray2(doc.items),
      /* [] */
      0
    ));
  ...
  var allModules = docs.map(function(param) {
    var submodules = fromArray(param[1].map(function(mod) {
      var items = keepMap(mod.items, function(item) {
        switch (item.kind) {
          case "value":
            var deprecated = item.deprecated;
            var id2 = item.id;
            var id$1 = id2.startsWith("RescriptCore") ? id2.replace("RescriptCore", "Core") : id2;
            return fromArray([
              [
            ...
            ].concat(deprecated !== void 0 ? [[
              "deprecated",
              deprecated
            ]] : []));
          case "type":
            var deprecated$1 = item.deprecated;
            var id$2 = item.id;
            var id$3 = id$2.startsWith("RescriptCore") ? id$2.replace("RescriptCore", "Core") : id$2;
            return fromArray([
              [
                "id",
                id$3
      ...
      var id = mod.id.startsWith("RescriptCore") ? "Core" : mod.id;
      var rest = fromArray([
        [
          "id",
          id
  ...
  allModules.forEach(function(param) {
    var topLevelName = param[0];
    var topLevelName$1 = topLevelName.startsWith("RescriptCore") ? "Core" : topLevelName;
    if (isDocFile) {
      console.log("here its a doc", Path2.join(destinationPath, topLevelName$1 + (isDocFile ? ".doc" : "") + ".json"));
    }
    Fs3.writeFileSync(Path2.join(destinationPath, topLevelName$1 + (isDocFile ? ".doc" : "") + ".json"), JSON.stringify(param[1], null, 2));
  });
  var joinPath = function(path, name) {
    return path.concat([name]).map(function(path2) {
      return path2;
    });
  };
  var getModules = function(_lst, _moduleNames, path) {
    while (true) {
      var moduleNames = _moduleNames;
      var lst = _lst;
      if (!lst) {
      ...
      var itemsList = fromArray2(items);
      var children = getModules(itemsList, [], joinPath(path, name));
      _moduleNames = [{
        name,
  ...
  var tocTree = docsDecoded.map(function(param) {
    var match = param[1];
    var name = match.name;
    var name$1 = name.startsWith("RescriptCore") ? "Core" : name;
    return [
      name$1,
      {
        name: name$1,
        groupName: param[0],
        path: [name$1],
        children: getModules(fromArray2(match.items), [], [name$1])
      }
    ];
  });
  console.log("tocTree", tocTree);
  Fs3.writeFileSync(Path2.join(destinationPath, "toc_tree.json"), getExn(JSON.stringify(fromArray(tocTree))));
}
function source_encode(v) {
  return fromArray(filterOptional([
    [
      "sidebarModuleName",
      false,
      stringToJson(v.sidebarModuleName)
    ],
    [
      "paths",
      false,
      function(extra) {
        return arrayToJson(stringToJson, extra);
      }(v.paths)
    ],
    [
      "expanded",
      false,
      boolToJson(v.expanded)
    ]
  ]));
}
function source_decode(v) {
  if (!Array.isArray(v) && (v === null || typeof v !== "object") && typeof v !== "number" && typeof v !== "string" && typeof v !== "boolean") {
    return error(void 0, "Not an object", v);
  }
  if (!(typeof v === "object" && !Array.isArray(v))) {
    return error(void 0, "Not an object", v);
  }
  var sidebarModuleName = stringFromJson(getWithDefault(get(v, "sidebarModuleName"), null));
  if (sidebarModuleName.TAG === "Ok") {
    var paths = function(extra) {
      return arrayFromJson(stringFromJson, extra);
    }(getWithDefault(get(v, "paths"), null));
    if (paths.TAG === "Ok") {
      var expanded = boolFromJson(getWithDefault(get(v, "expanded"), null));
      if (expanded.TAG === "Ok") {
        return {
          TAG: "Ok",
          _0: {
            sidebarModuleName: sidebarModuleName._0,
            paths: paths._0,
            expanded: expanded._0
          }
        };
      }
      var e = expanded._0;
  ...
  var e$2 = sidebarModuleName._0;
  return {
    TAG: "Error",
    _0: {
      path: ".sidebarModuleName" + e$2.path,
      message: e$2.message,
      value: e$2.value
    }
  };
}
function configFile_encode(v) {
  return fromArray(filterOptional([
    [
      "platform",
      false,
      stringToJson(v.platform)
    ],
    [
      "sources",
      false,
      function(extra) {
        return arrayToJson(source_encode, extra);
      }(v.sources)
    ],
    [
      "rescriptDirectory",
      false,
      stringToJson(v.rescriptDirectory)
    ],
    [
      "metadataDirectory",
      false,
      stringToJson(v.metadataDirectory)
    ],
    [
      "rootFilePath",
      false,
      stringToJson(v.rootFilePath)
    ]
  ]));
}
function configFile_decode(v) {
  if (!Array.isArray(v) && (v === null || typeof v !== "object") && typeof v !== "number" && typeof v !== "string" && typeof v !== "boolean") {
    return error(void 0, "Not an object", v);
  }
  if (!(typeof v === "object" && !Array.isArray(v))) {
  ...
  var platform = getWithDefault(map5(get(v, "platform"), stringFromJson), {
    TAG: "Ok",
    _0: "darwinarm64"
  });
  if (platform.TAG === "Ok") {
    var sources = function(extra) {
      return arrayFromJson(source_decode, extra);
    }(getWithDefault(get(v, "sources"), null));
    if (sources.TAG === "Ok") {
      var rescriptDirectory = stringFromJson(getWithDefault(get(v, "rescriptDirectory"), null));
      if (rescriptDirectory.TAG === "Ok") {
        var metadataDirectory = stringFromJson(getWithDefault(get(v, "metadataDirectory"), null));
        if (metadataDirectory.TAG === "Ok") {
          var rootFilePath = stringFromJson(getWithDefault(get(v, "rootFilePath"), null));
          if (rootFilePath.TAG === "Ok") {
            return {
              TAG: "Ok",
              _0: {
                platform: platform._0,
                sources: sources._0,
                rescriptDirectory: rescriptDirectory._0,
                metadataDirectory: metadataDirectory._0,
                rootFilePath: rootFilePath._0
              }
            };
          }
          var e = rootFilePath._0;
          return {
            TAG: "Error",
            _0: {
              path: ".rootFilePath" + e.path,
              message: e.message,
              value: e.value
            }
          };
        }
        var e$1 = metadataDirectory._0;
        return {
          TAG: "Error",
          _0: {
            path: ".metadataDirectory" + e$1.path,
            message: e$1.message,
            value: e$1.value
          }
        };
      }
      var e$2 = rescriptDirectory._0;
      return {
        TAG: "Error",
        _0: {
          path: ".rescriptDirectory" + e$2.path,
          message: e$2.message,
          value: e$2.value
        }
      };
...
var filepath = getOr(process.argv[2], Path2.join("doc-generator-config.json"));
var text7 = Fs3.readFileSync(filepath);
try {
  json = JSON.parse(text7, void 0);
  map6(mapError(configFile_decode(json), function(e) {
    console.log("error while decoding config", e);
    return e;
  }), function(r) {
    dummy(r.rootFilePath);
    if (Fs3.existsSync(r.metadataDirectory)) {
      Child_process.execSync("rm -rf " + r.metadataDirectory);
    }
    if (Fs3.existsSync(r.rescriptDirectory)) {
      Child_process.execSync("rm -rf " + r.rescriptDirectory);
    }
    Fs3.mkdirSync(r.rescriptDirectory);
    Fs3.mkdirSync(r.metadataDirectory);
    var sourceFiles = reduce2(r.sources.map(function(s2) {
      return s2.paths.map(function(p2) {
        return [
          s2.sidebarModuleName,
          p2
        ];
      });
    }).flat(), [], function(acc, param) {
      var group = param[0];
      return acc.concat(getAllFiles(param[1]).map(function(p2) {
        return [
          group,
          p2
        ];
    ...
    var docSources = sourceFiles.filter(function(param) {
      return param[1].includes("_doc.res");
    });
    var sources = sourceFiles.filter(function(param) {
      return !param[1].includes("_doc.res");
    });
    run(r.metadataDirectory, docSources, r.platform, true);
    run(r.metadataDirectory, sources, r.platform, void 0);
    var metadataFiles = Fs3.readdirSync(r.metadataDirectory);
    var tocTreeFile = r.metadataDirectory + "/toc_tree.json";
    var tocTree = getOr(Decode.object(JSON.parse(Fs3.readFileSync(tocTreeFile), void 0)), {});
    metadataFiles.filter(function(x) {
      return !x.includes(".doc.json");
    }).forEach(function(fileWExt) {
      var filepath2 = r.metadataDirectory + "/" + fileWExt;
      var moduleName = fileWExt.replace(".json", "");
      if (filepath2.includes("toc_tree")) {
        return;
      }
      console.log("Generating module -----------  " + moduleName + " from file path " + filepath2);
      try {
        var docFileASTPath = r.metadataDirectory + "/" + moduleName + "_doc.doc.json";
        var sourceContent = getOr(Decode.object(JSON.parse(Fs3.readFileSync(filepath2), void 0)), {});
        var docFileAST;
        if (Fs3.existsSync(docFileASTPath)) {
          docFileAST = some(getOr(Decode.object(JSON.parse(Fs3.readFileSync(docFileASTPath), void 0)), {}));
        }
        var destnFilePath = r.rescriptDirectory + "/" + capitalize(moduleName) + "_Gen_Doc.res";
        console.log("**** Generating MDX Doc at ", destnFilePath, "");
        var docFileContent = mapOr(docFileAST, "", function(param) {
          return mapOr(sourceFiles.find(function(param2) {
            return param2[1].endsWith("/" + moduleName + "_doc.res");
          }), "", function(param2) {
            var file = Fs3.readFileSync(param2[1], {
              encoding: "utf8"
            });
            return file.replaceAll(/@ocaml\.doc\(`[^`]*`\)/g, "");
          });
        });
        $$default(moduleName, sourceContent, tocTree, destnFilePath, docFileAST, docFileContent);
        return;
      } catch (raw_err) {
        var err = internalToOCamlException(raw_err);
        console.log("Error while reading filepath " + filepath2, err);
        return;
      }
    });
    create(tocTree, r.rootFilePath, r.sources);
  });
} catch (raw_err) {
  err = internalToOCamlException(raw_err);
  console.log("error while decoding config file", err);
}
var json;
var err;
var Docgen;
