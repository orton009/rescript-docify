...
var generateMdx_bs_exports = {};
__export(generateMdx_bs_exports, {
  Docgen: () => Docgen,
  astTransform: () => astTransform,
  content: () => content3,
  default: () => $$default,
  getModuleJson: () => getModuleJson,
  stylize: () => stylize,
  tocTree: () => tocTree
});
module.exports = __toCommonJS(generateMdx_bs_exports);
...
function element(state, node2) {
  ...
  if (result.tagName === "template") {
    ...
    if (startTag2 && endTag2 && state.file) {
      content4.position = { start: startTag2.end, end: endTag2.start };
    }
    result.content = content4;
  }
  state.schema = schema;
  return result;
}
...
var defaultTreeAdapter = {
  ...
  insertTextBefore(parentNode, text7, referenceNode) {
    ...
    if (prevNode && defaultTreeAdapter.isTextNode(prevNode)) {
      ...
      defaultTreeAdapter.insertBefore(parentNode, createTextNode(text7), referenceNode);
    }
  },
...
var Parser = class {
  ...
  _insertTemplate(token) {
    ...
    const content4 = this.treeAdapter.createDocumentFragment();
    this.treeAdapter.setTemplateContent(tmpl, content4);
    this._attachElementToTree(tmpl, token.location);
    this.openElements.push(tmpl, token.tagID);
    if (this.options.sourceCodeLocationInfo)
      this.treeAdapter.setNodeSourceCodeLocation(content4, null);
  }
...
function resolveToSetextUnderline(events, context) {
  ...
  if (definition2) {
    ...
    events.splice(definition2 + 1, 0, ["exit", events[content4][1], context]);
    events[content4][1].end = Object.assign({}, events[definition2][1].end);
  } else {
...
function initializeFactory(field) {
  ...
  function initializeText(effects) {
    ...
    return start;
    function start(code2) {
      return atBreak(code2) ? text7(code2) : notText(code2);
    }
    ...
    function hook(constructs2, returnState, bogusState) {
      ...
      function handleMapOfConstructs(map) {
        ...
        function start(code2) {
          ...
          const all6 = code2 !== null && map.null;
          const list3 = [
...
function footer(state) {
  ...
  while (++referenceIndex < state.footnoteOrder.length) {
    ...
    const tail = content4[content4.length - 1];
    if (tail && tail.type === "element" && tail.tagName === "p") {
      ...
      content4.push(...backReferences);
    }
    const listItem2 = {
      type: "element",
      tagName: "li",
      properties: { id: clobberPrefix + "fn-" + safeId },
      children: state.wrap(content4, true)
    };
...
// scripts/generateMdx.bs.js
var astTransform = function mapper(ast) {
  console.log(ast);
  let mapChildren = () => {
    if (ast.children.length > 0) {
      let childrens = [];
      for (let c of ast.children) {
        childrens.push(mapper(c));
      }
      ast.childrens = childrens;
    }
  };
  if (ast.type == "root") {
    mapChildren();
  ...
  if (ast.type == "text") {
    ast.value = '{"' + ast.value + '"->React.string}';
  }
  if (ast.type == "element") {
    switch (ast.tagName) {
      case "cite":
...
async function stylize(content4) {
  var rendered = async (unified2, remarkParse2, remarkRehype2, rehypeRaw2, rehypeStringify2, content5) => {
    function logContent() {
      return (tree) => {
        tree = astTransform(tree);
        return tree;
      };
    }
    const file = await unified2().use(remarkParse2).use(remarkRehype2, { "allowDangerousHtml": true }).use(logContent).use(rehypeStringify2, { "allowDangerousHtml": true }).process(content5);
    return String(file);
  };
  return await rendered(unified, remarkParse, remarkRehype, rehypeRaw, rehypeStringify, content4);
}
function getModuleJson(moduleName, mainModule, tree) {
  return entries(mainModule).map(function(param) {
    var content4 = param[1];
    var match = decodeFromJson(content4);
    var id;
    if (!Array.isArray(content4) && (content4 === null || typeof content4 !== "object") && typeof content4 !== "number" && typeof content4 !== "string" && typeof content4 !== "boolean" || !(typeof content4 === "object" && !Array.isArray(content4))) {
      id = "";
    } else {
      var match$1 = get(content4, "id");
...
async function $$default(moduleName, mainModule, tree, destinationFilePath) {
  var moduleResult = getModuleJson(moduleName, mainModule, tree);
  var encodeDocstrings = async function(s2) {
...
var content3 = Fs.readFileSync("/Users/drupadkumar.singh/Repo/rescript-euler-dashboard/packages/crafter/docs_metadata/cf_color.json");
var tocTree = Fs.readFileSync("/Users/drupadkumar.singh/Repo/rescript-euler-dashboard/packages/crafter/docs_metadata/toc_tree.json");
$$default("cf_badgevariants", JSON.parse(content3), JSON.parse(tocTree), "/Users/drupadkumar.singh/Repo/rescript-euler-dashboard/packages/crafter/docs/CF_Color_Doc.doc.res").then(function(result) {
  console.log(result);
